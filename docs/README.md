...
日志文件大小限制为 5MB，超过限制会自动轮转，防止占用过多存储空间。
...
- **GPU配置页面**：显示和编辑当前配置的频率、电压和内存频率档位
- **游戏列表页面**：显示和编辑已配置的游戏包名列表
...
**Q: 如何调整日志等级？**
A: 使用命令 `./action.sh log_level [debug|info|warn|error]` 或通过 WebUI 界面的设置页面进行调整。调整后需要重启模块才能生效。
