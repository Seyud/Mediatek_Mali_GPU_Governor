...
export class LogManager {
	...
	setupEventListeners() {
		...
		logTabBtns.forEach(btn => {
			btn.addEventListener('click', () => {
				...
				setTimeout(() => { this.loadLog().then(() => { if (this.logContent) this.logContent.style.opacity = '1'; }); }, 100);
			});
		});
	}
	...
	async loadLog() {
		try {
			...
			if (this.logContent) this.logContent.textContent = getTranslation('log_loading', {}, this.currentLanguage);
			const { errno, stdout } = await exec(`cat ${PATHS.LOG_PATH}/${selectedLog} 2>/dev/null || echo "日志文件不存在"`);
			if (stdout.trim() === '日志文件不存在') {
				if (this.logContent) this.logContent.textContent = getTranslation('log_not_found', {}, this.currentLanguage);
				return;
			}
			if (errno === 0) {
				const lines = stdout.trim().split('\n');
				const lastLines = lines.slice(-100).join('\n');
				if (this.logContent) this.logContent.textContent = lastLines || getTranslation('log_empty', {}, this.currentLanguage);
				if (this.logContent) this.logContent.scrollTop = this.logContent.scrollHeight;
			} else if (this.logContent) this.logContent.textContent = getTranslation('log_not_found', {}, this.currentLanguage);
		} catch (error) {
