...
export class ModalManager {
	...
	init() { this.setupEventListeners(); this.setupDdrSelector(); }
	...
	setupEventListeners() {
		...
		if (this.deleteItemBtn) this.deleteItemBtn.addEventListener('click', () => this.deleteConfigItem());
		window.addEventListener('click', (event) => { if (event.target === this.editConfigModal) this.closeModal(); });
		window.addEventListener('keydown', (event) => { if (event.key === 'Escape' && this.editConfigModal && this.editConfigModal.style.display === 'block') this.closeModal(); });
	}
	...
	setupDdrSelector() {
		...
		this.ddrContainer.addEventListener('click', () => {
			...
			if (this.ddrContainer!.classList.contains('open')) {
				setTimeout(() => {
					...
					options.forEach((option, index) => { setTimeout(() => { (option as HTMLElement).style.opacity = '1'; (option as HTMLElement).style.transform = 'translateY(0)'; }, index * 50); });
				}, 10);
			} else {
				const options = this.ddrContainer!.querySelectorAll('.option');
				options.forEach(option => { (option as HTMLElement).style.opacity = '0'; (option as HTMLElement).style.transform = 'translateY(-10px)'; });
				setTimeout(() => { this.ddrContainer!.classList.remove('open'); }, 150);
			}
		});
		...
		ddrOptionElements.forEach(option => {
			option.addEventListener('click', (e) => {
				...
				const options = this.ddrContainer!.querySelectorAll('.option');
				options.forEach(o => { (o as HTMLElement).style.opacity = '0'; (o as HTMLElement).style.transform = 'translateY(-10px)'; });
				setTimeout(() => { this.ddrContainer!.classList.remove('open'); }, 150);
				setTimeout(() => { options.forEach(o => { (o as HTMLElement).style.opacity = ''; (o as HTMLElement).style.transform = ''; }); }, 300);
			});
		});
	}

	openModal(config: any = null, index = -1) {
		if (!this.editConfigModal) { console.error('模态框元素不存在'); return; }
	...
	closeModal() { if (this.editConfigModal) this.editConfigModal.style.display = 'none'; }

	setDdrValue(ddrValue: number) {
		if (this.selectedDdr) {
			const ddrOptions = document.querySelectorAll('#ddrOptions .option');
			ddrOptions.forEach(option => {
				const optionValue = parseInt(option.getAttribute('data-value') || '0');
				if (optionValue === ddrValue) { this.selectedDdr!.textContent = option.textContent; option.classList.add('selected'); }
				else option.classList.remove('selected');
			});
		}
	}

	getDdrValue(): number { if (this.selectedDdr) { const ddrText = this.selectedDdr.textContent || '999'; return parseInt(ddrText.split(' ')[0]); } return 999; }
	...
	saveConfigItem() {
		...
		if (!freq || isNaN(freq)) { toast(getTranslation('toast_freq_invalid', {}, this.currentLanguage)); return; }
	...
	setSaveCallback(cb: (config: any, index: number) => void) { this.onSaveCallback = cb; }
	setDeleteCallback(cb: (index: number) => void) { this.onDeleteCallback = cb; }
