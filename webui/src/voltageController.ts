import { VOLT_LIST, VOLT_SETTINGS } from "./constants";
...
export class VoltageController {
	...
	decreaseVolt() {
		const newVolt = this.currentVoltValue - VOLT_SETTINGS.VOLT_STEP;
	...
	increaseVolt() {
		const newVolt = this.currentVoltValue + VOLT_SETTINGS.VOLT_STEP;
		if (newVolt <= (VOLT_SETTINGS.MAX_VOLT as number)) {
			this.currentVoltValue = newVolt;
			this.updateVoltDisplay();
			return true;
		}
	...
	updateVoltDisplay() {
		if (this.selectedVolt) this.selectedVolt.textContent = String(this.currentVoltValue);
		if (this.voltSelect) {
			const voltOption = Array.from(this.voltSelect.options).find(
				(o) => parseInt(o.value, 10) === this.currentVoltValue
			);
			if (voltOption) this.voltSelect.value = voltOption.value;
			else {
				const option = document.createElement("option");
				option.value = String(this.currentVoltValue);
				option.textContent = String(this.currentVoltValue);
				this.voltSelect.appendChild(option);
				this.voltSelect.value = String(this.currentVoltValue);
			}
			if (this.voltDecreaseBtn)
				(this.voltDecreaseBtn as HTMLButtonElement).disabled =
					this.currentVoltValue <= VOLT_SETTINGS.MIN_VOLT;
			if (this.voltIncreaseBtn)
				(this.voltIncreaseBtn as HTMLButtonElement).disabled =
					this.currentVoltValue >= VOLT_SETTINGS.MAX_VOLT;
		}
	}
	initVoltSelect() {
		if (!this.voltSelect || !this.selectedVolt) return;
		this.voltSelect.innerHTML = "";
		VOLT_LIST.forEach((volt) => {
			const opt = document.createElement("option");
			opt.value = String(volt);
			opt.textContent = String(volt);
			this.voltSelect?.appendChild(opt);
		});
		this.currentVoltValue = VOLT_SETTINGS.MAX_VOLT;
		this.selectedVolt.textContent = String(this.currentVoltValue);
		this.voltSelect.value = String(this.currentVoltValue);
		this.updateVoltDisplay();
	}
	setupVoltageEvents() {
		if (this.voltageEventsInitialized) return;
		if (this.voltDecreaseBtn) {
	...
	setupTouchEvents() {
		if (this.voltDecreaseBtn) {
			this.voltDecreaseBtn.addEventListener(
				"touchstart",
				(e) => {
					e.preventDefault();
					this.isLongPress = false;
		...
		if (this.voltIncreaseBtn) {
			this.voltIncreaseBtn.addEventListener(
				"touchstart",
				(e) => {
					e.preventDefault();
					this.isLongPress = false;
