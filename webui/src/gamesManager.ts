...
interface GameItem { package: string; mode: string; }
...
export class GamesManager {
	...
	setupEventListeners() {
		...
		if (this.saveGamesBtn) this.saveGamesBtn.addEventListener('click', () => this.saveGamesToFile());
		if (this.closeGameModalBtn) this.closeGameModalBtn.addEventListener('click', () => this.closeGameModal());
		if (this.cancelGameBtn) this.cancelGameBtn.addEventListener('click', () => this.closeGameModal());
		if (this.saveGameBtn) this.saveGameBtn.addEventListener('click', () => this.saveGameItem());
		window.addEventListener('click', (event) => { if (event.target === this.editGameModal) this.closeGameModal(); });
		window.addEventListener('keydown', (event) => { if (event.key === 'Escape' && this.editGameModal && this.editGameModal.style.display === 'block') this.closeGameModal(); });
	...
	parseTomlGames(tomlString: string) {
		const games: any[] = []; const lines = tomlString.split('\n'); let currentGame: any = null;
		for (const line of lines) {
			const trimmed = line.trim();
			if (!trimmed || trimmed.startsWith('#')) continue;
			if (trimmed.startsWith('[[games]]')) { if (currentGame) games.push(currentGame); currentGame = {}; continue; }
			if (currentGame && trimmed.includes('=')) { const [k,v] = trimmed.split('='); const ck = k.trim(); const cv = v.trim().replace(/"/g,''); if (ck==='package') currentGame.package=cv; else if (ck==='mode') currentGame.mode=cv; }
		}
	...
	refreshGamesList() {
		if (!this.gamesList) return; this.gamesList.innerHTML='';
		if (this.gamesListData.length===0) { this.gamesList.innerHTML = `<li class="loading-text">${getTranslation('config_games_not_found', {}, this.currentLanguage)}</li>`; return; }
		this.gamesListData.forEach((game, index) => {
			const li = document.createElement('li');
			const gameText = document.createElement('span');
			const modeText = this.getModeText(game.mode || 'balance');
			gameText.textContent = game.package ? `${game.package} (${modeText})` : (game as any).trim?.();
			li.appendChild(gameText);
			const buttonContainer = document.createElement('div'); buttonContainer.className='game-button-container';
			const editBtn = document.createElement('button'); editBtn.className='game-edit-btn'; editBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M3,17.25V21h3.75L17.81,9.94L14.06,6.19L3,17.25M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.13,5.12L18.88,8.87L20.71,7.04Z"/></svg>`; editBtn.title='编辑'; editBtn.onclick = (e) => { e.stopPropagation(); this.editGameItem(index); }; buttonContainer.appendChild(editBtn);
			const deleteBtn = document.createElement('button'); deleteBtn.className='game-delete-btn'; deleteBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>`; deleteBtn.title='删除'; deleteBtn.onclick = (e) => { e.stopPropagation(); this.deleteGameItem(index); }; buttonContainer.appendChild(deleteBtn);
			li.appendChild(buttonContainer); this.gamesList!.appendChild(li);
		});
	}

	getModeText(mode: string) {
		const modeTexts: Record<Lang, Record<string,string>> = { zh: { powersave:'省电', balance:'均衡', performance:'性能', fast:'极速' }, en: { powersave:'Power Saving', balance:'Balance', performance:'Performance', fast:'Fast' } };
		return modeTexts[this.currentLanguage][mode] || modeTexts[this.currentLanguage]['balance'];
	}

	openGameModal() { if (this.packageNameInput) this.packageNameInput.value=''; if (this.gameModeSelect) this.gameModeSelect.value='balance'; this.updateGameModeSelectDisplay('balance'); if (this.editGameModal) this.editGameModal.style.display='block'; this.editingIndex=-1; }
	editGameItem(index: number) { if (index<0||index>=this.gamesListData.length) return; const game=this.gamesListData[index]; if (this.packageNameInput) this.packageNameInput.value=game.package||''; if (this.gameModeSelect) this.gameModeSelect.value=game.mode||'balance'; this.updateGameModeSelectDisplay(game.mode||'balance'); if (this.editGameModal) this.editGameModal.style.display='block'; this.editingIndex=index; }
	closeGameModal() { if (this.editGameModal) this.editGameModal.style.display='none'; this.editingIndex=-1; }
	saveGameItem() { if (!this.packageNameInput) return; const packageName=this.packageNameInput.value.trim(); const gameMode=this.gameModeSelect?this.gameModeSelect.value:'balance'; if (!packageName) { toast(getTranslation('toast_game_invalid', {}, this.currentLanguage)); return; } if (this.editingIndex===-1) { if (this.gamesListData.some(g=>g.package===packageName)) { toast(getTranslation('toast_game_exists', {}, this.currentLanguage)); return; } this.gamesListData.push({ package: packageName, mode: gameMode }); } else { this.gamesListData[this.editingIndex].package=packageName; this.gamesListData[this.editingIndex].mode=gameMode; } this.closeGameModal(); this.refreshGamesList(); if (this.editingIndex===-1) toast(getTranslation('toast_game_added', {}, this.currentLanguage)); }
