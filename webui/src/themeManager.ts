...
export class ThemeManager {
	...
	setupEventListeners() {
		...
		if (this.followSystemThemeSuperSwitch) {
			this.followSystemThemeSuperSwitch.addEventListener('click', (e) => this.handleFollowSystemToggleClick(e));
			...
			this.followSystemThemeSuperSwitch.setAttribute('aria-checked', this.followSystemThemeToggle?.checked ? 'true' : 'false');
		}
		if (this.followSystemThemeToggle) {
			this.followSystemThemeToggle.addEventListener('change', () => this.handleFollowSystemChange());
		}
	}
	...
	handleFollowSystemChange() {
		...
		if (this.followSystemThemeSuperSwitch) this.followSystemThemeSuperSwitch.setAttribute('aria-checked', isFollowSystem ? 'true' : 'false');
		if (this.followSystemThemeSuperSwitch) {
			this.followSystemThemeSuperSwitch.style.transform = 'scale(0.98)';
			setTimeout(() => { if (this.followSystemThemeSuperSwitch) this.followSystemThemeSuperSwitch.style.transform = ''; }, 150);
		}
		if (isFollowSystem) {
