...
async function run() {
	let original;
	try {
		original = readFileSync(INDEX_HTML, "utf-8");
	} catch {
		console.error("[postbuild-htmlmin] 读取 index.html 失败");
		process.exit(1);
	}
	...
	if (afterSize < beforeSize) {
		...
		console.log(
			`[postbuild-htmlmin] 压缩完成: ${beforeSize}B -> ${afterSize}B (节省 ${(100 - (afterSize / beforeSize) * 100).toFixed(2)}%)`
		);
	} else {
